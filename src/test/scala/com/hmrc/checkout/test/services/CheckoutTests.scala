...
class CheckoutTests extends FlatSpec with Matchers {
  private[this] val prices = Map("apple" -> BigDecimal(60), "orange" -> BigDecimal(25))
  private[this] val priceLookup: String => Option[BigDecimal] = item => prices.get(item.toLowerCase.trim)

  private[this] val discountFunctions = Map[String, (Int, BigDecimal) => BigDecimal](
    "apple" -> ((quantity, price) => if (quantity > 1) (quantity % 2) * price else 0),
    "orange" -> ((quantity, price) => Math.floor(quantity / 3) * price)
  )

  private[this] val discounter: String => (Int, BigDecimal) => BigDecimal =
    discountFunctions.getOrElse(_, (_, _) => BigDecimal(0))

  it should "return nothing for an empty basket" in {
    val items = Seq()
    val checkout = Checkout(priceLookup, discounter)
